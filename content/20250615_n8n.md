...
For starters, it can contain following variables:
...
```bash
  sudo docker run -d --restart unless-stopped --name n8n --env-file ~/.env -p 5678:5678 -v n8n_data:/home/node/.n8n docker.n8n.io/n8nio/n8n:stable
```
...
Standard directives that must be in the configuration file for nginx to forward requests to an app in a container are:
...
But for n8n it's not enough. In order for specifically n8n to work properly, we need more directives, otherwise the app will complain about not being able to connect to the server (see [this discussion](https://community.n8n.io/t/error-while-executing-workflow/28560/8)).

Here is my updated nginx config file:
